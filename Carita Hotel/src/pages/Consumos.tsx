        ...
        <div className="flex flex-col gap-6 lg:flex-row">
          <div className="flex-1">
            <div className="mb-4 rounded-lg border border-neutral-200 p-4 dark:border-neutral-800">
              <p className="text-xs text-neutral-500 dark:text-neutral-400">Total de consumos</p>
              <p className="mt-1 text-lg font-semibold tabular-nums text-neutral-900 dark:text-neutral-100">S/ {total.toFixed(2)}</p>
            </div>
            ...
            <div className="overflow-x-auto rounded-lg border border-neutral-200 dark:border-neutral-800">
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                  {consumos.map((c) => (
                    <tr key={c.id}>
                      <td className="px-4 py-3 text-neutral-600 dark:text-neutral-400">{c.fechaHora.slice(0, 16).replace('T', ' ')}</td>
                      <td className="px-4 py-3 text-neutral-600 dark:text-neutral-400">{c.descripcion}</td>
                      <td className="px-4 py-3 tabular-nums text-neutral-600 dark:text-neutral-400">{c.cantidad}</td>
                      <td className="px-4 py-3 tabular-nums text-neutral-600 dark:text-neutral-400">S/ {c.precioUnitario.toFixed(2)}</td>
                      <td className="px-4 py-3 tabular-nums text-neutral-600 dark:text-neutral-400">S/ {c.subtotal.toFixed(2)}</td>
                    </tr>
                  ))}
                  {consumos.length === 0 && (
                    <tr>
          ...
          <form onSubmit={handleSubmit} className="flex w-full flex-col gap-4 lg:w-72">
            <h3 className="text-sm font-medium text-neutral-900 dark:text-neutral-100">Registrar consumo</h3>
